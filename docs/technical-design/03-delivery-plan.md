...
Post-release CLIP-IMPORT-01 adds the bounded Studio collection/detail queries and atomic owner-or-saved exact-version import authority, including editable contribution workspaces. CLIP-IMPORT-02 is its completed responsive Studio consumer. COLLECTION-01 unifies that drawer with canonical `/library/collection`: My clips is latest-per-owned-pattern presentation, Saved clips remains exact-version bookmarks, historical versions remain immutable and outside the ordinary owned list, and both surfaces use bounded metadata with no notes plus on-demand detail. The full page loads that detail near the viewport and reuses the public-library note-roll/playhead preview before playback; Studio keeps its compact deliberate-preview interaction. `/library/saved` redirects to the saved source.
