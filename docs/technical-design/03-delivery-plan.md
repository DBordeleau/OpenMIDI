...
Post-release CLIP-IMPORT-01 adds the bounded Studio collection/detail queries and atomic owner-or-saved exact-version import authority, including editable contribution workspaces, without changing visible Studio UI. CLIP-IMPORT-02 is the completed ordered consumer: its responsive Studio drawer uses the returned canonical manifest, hash, lock, track/clip IDs, and validated playback payload instead of reloading the route or inventing a second import path.
