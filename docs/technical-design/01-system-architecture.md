...
- **Challenge lifecycle and entries (implemented through CHALLENGE-02):** administrator commands create a stable identity plus append-only challenge versions containing presentation, frozen UTC phase boundaries, judging-credit snapshots, an optional exact reusable public starter revision, and validated canonical constraint-v1 JSON plus hash. Public `scheduled`, `open`, and `voting` phases derive from those boundaries, so challenge opening/closure requires no cron job; cancellation is an explicit audited administrator transition. An active owner preflights only a current immutable revision. TypeScript provides advisory normalized-fact feedback, while Postgres independently extracts arrangement facts and re-evaluates every rule before an idempotent first submission or optimistic atomic replacement. Entries pin the exact challenge version/revision and challenge-display attestation. Before voting opens, public entry list/detail functions return no identities or counts; afterward, only the active moderation-visible entry and its exact browser-local preview projection are available, without widening private project, download, editor, or reuse authority. CHALLENGE-03 adds private votes, moderation administration, append-only finalized results/completion, and the one featured landing/dashboard projection.
