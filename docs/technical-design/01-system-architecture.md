...
- **Challenges (planned, worker-ready):** administrator commands create an identity plus append-only challenge versions containing presentation, frozen UTC phase boundaries, judging credits, an optional exact public starter revision, and a validated constraint-v1 document. Public `scheduled`, `open`, and `voting` phases derive from those boundaries, so challenge opening/closure requires no cron job; cancellation and completion remain explicit audited administrator transitions. CHALLENGE-02 will pin entries to one exact challenge version and immutable project revision, expose only a challenge-scoped safe public arrangement projection, and revalidate canonical normalized arrangement facts in Postgres. CHALLENGE-03 adds private votes, moderation, append-only finalized results, and the one featured landing/dashboard projection. Pre-close entries/vote totals, private projects outside an entry snapshot, and administration data never enter public reads.
