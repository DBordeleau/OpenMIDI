...
- Generate waveform peaks and an optional compressed preview asynchronously. OPT-04 peak generation stays in the upload worker and finalizes only a small browser-produced derivative; no request path decodes or transcodes audio. Do not make Vercel request duration the processing contract.
