...
- **Library saved clips and reuse (implemented through COLLECTION-01):** canonical `/library/collection` has explicit **My clips** and **Saved clips** sources; `/library/saved` redirects to the saved source. My clips selects one latest immutable version for each active actor-owned pattern identity. Saved clips preserves exact bookmarks without copying notes or ownership, including older versions of owned patterns. Every saved save/import/fork/editor/export command still rechecks the exact commercially reusable listing, immutable CC BY terms, moderation, lifecycle, and actor authority. Preview and attributed Standard MIDI File export remain browser-local.
- **Studio clip collection and import (implemented through COLLECTION-01):** one bounded Auth-dependent projection returns latest-per-owned-pattern metadata and exact saved bookmarks without note arrays. It deduplicates only the same exact latest owned-and-saved version with owner authority, retains saved availability/provenance context, and leaves historical versions untouched. The full collection page and responsive **Add from clips** drawer fetch only the selected source and use bounded URL/UI search. The page loads exact structured MIDI as cards approach the viewport and passes it to the established public-library piano-roll preview, while Studio retains deliberate detail loading inside its drawer. Atomic import authority remains unchanged across actor-owned and eligible contribution workspaces.
