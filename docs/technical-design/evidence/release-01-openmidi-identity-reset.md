...
It preserves Auth users, profiles, administrator roles and invitations, beta feedback and its administrator audit, avatar assets/versions/jobs, profile-targeted moderation and holds, licenses, genres, tags, instruments, MIDI/library preset catalogs, library lookup catalogs, reserved usernames, and badge definition/version catalogs. It then installs the canonical engine constraints, stored-function bodies, comments, and synthesis preset engine version.
...
- A populated local reconciliation preserves an Auth-backed profile, administrator membership, beta feedback, avatar asset/version, invitation/catalog state, while deleting its project, workspace, and pattern.
...
- A clean local reset and `npm run db:check` pass all 28 pgTAP files and 900 assertions with current generated types.
