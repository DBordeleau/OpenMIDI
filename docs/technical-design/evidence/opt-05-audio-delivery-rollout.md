...
- `npm run test:e2e:studio` was attempted twice and is unavailable in this environment. Both runs reached the manifest-first editable studio and returned authorized audio-source descriptors, but the persisted-waveform assertion timed out while the track stayed in `loading`. One focused attempt replaced the existing in-page local-Storage substitution with Playwright request routing; it reached the identical state, so the unsuccessful test edit was reverted and the repository's two-attempt ceiling stopped further retries. Lower-layer adapter/peak tests passed in `npm run check`, and the same targeted journey is recorded passing after the OPT-04 harness repair.
