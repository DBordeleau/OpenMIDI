...
- Local Chromium studio journey: verifies the real source and peak objects in Storage during fixture preflight, calls and validates the real authorized descriptor/signing endpoint, rewrites only the fixture delivery URLs to a test-owned loopback server, delays canonical source delivery, and asserts the accessible persisted-waveform state appears before real browser decode/playback readiness.

The fixture loopback server is local test infrastructure only and serves the exact bytes already verified through private Storage preflight. Production code still fetches short-lived signed private URLs, and hosted/manual playback remains the authority for the real transport path. This boundary prevents agents from repeatedly spending the test retry budget on a diagnosed local Storage body-stream issue while preserving meaningful browser coverage above that transport.
