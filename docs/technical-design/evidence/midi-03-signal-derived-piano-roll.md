...
The standalone My stems editor now owns a viewport-rendered piano roll without adopting Signal runtime state, stores, routing, transport, or file authority. Signal remains the interaction reference pinned and attributed by MIDI-01. Canvas state is visual and transient; every committed edit becomes a platform semantic command over canonical `MidiNoteV1` data.
