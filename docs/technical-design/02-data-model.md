...
Administrator membership lives in unexposed `private.app_admins`; the no-argument `private.is_admin()` helper checks only the current authenticated user and grants no automatic RLS bypass. The display-only `assert_viewer_admin()` probe returns a boolean for dashboard presentation, including `false` for an ordinary active member; it is not mutation authority. Administrator commands call the separate exception-raising `private.assert_admin_actor()` guard and fail closed unless the caller is an active, completed administrator.

`private.signup_invitations` remains the only beta admission allowlist and is directly unreadable/unwritable by application roles. `activate_signup_invitation(text)` is the narrow authenticated command: after the private administrator guard succeeds, it normalizes and validates one address, serializes on that natural key, inserts a missing row, returns an idempotent success for an active row, or clears `revoked_at` for a revoked row. New and reactivated rows use the existing `created_by`/`created_at` fields as current activation actor/time; no second event ledger, public view, invitation email, or Auth user is created. The Before User Created hook still decides whether a matching Google identity may create an account.

`touch_viewer_activity()` writes only the verified caller and only when the prior value is at least 15 minutes old. Avatar reservations create `assets(kind='image')` private originals plus one `profile_avatar_versions` candidate; trusted finalization atomically swaps the safe profile pointer and queues superseded private/public objects for lease-bound cleanup.
