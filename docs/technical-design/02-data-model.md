...
CHALLENGE-01 and CHALLENGE-02 implement the lifecycle and exact-entry foundations below. Result and award boundaries remain accepted sequential extensions whose exact schema belongs to their detailed plan and forward migration.

- **Challenges (lifecycle and entries implemented):** `challenges` holds the stable lifecycle pointer; `challenge_versions` and `challenge_judge_credits` are append-only immutable snapshots containing frozen UTC boundaries, prompt/presentation, host/judge credit names, an optional exact starter revision, and validated canonical constraint-v1 JSON plus SHA-256. `challenge_entries` pins the exact current project revision, challenge version, display attestation, identity/attribution snapshots, normalized facts, and authoritative evaluation/hash. A partial unique index permits one active entry per entrant/challenge; replacement appends a row and closes the prior active row in one optimistic idempotent command. `private.challenge_entry_commands` retains command audit/idempotency authority. Time-derived functions return no pre-voting entry identities/counts, then expose only active visible entry snapshots and exact structured preview data through challenge-scoped projections. Source project visibility and library/download/editor rights do not change. Later sequential migrations add private votes, challenge/entry/vote moderation administration, and append-only finalized-result versions.
