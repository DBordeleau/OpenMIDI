...
- `list_studio_clip_collection` returns at most 100 metadata rows without notes. Owned candidates select one latest immutable version per non-deleted actor-owned `midi_patterns` identity and may include a bounded version count; saved candidates preserve exact bookmark version IDs and availability. Only the same exact latest owned-and-saved version deduplicates with owner authority, while an older explicitly saved owned version remains distinct. Existing owner/update and unique `(midi_pattern_id, version_number)` indexes support the projection without a new speculative index. `get_studio_clip_detail` rechecks exact authority before returning one bounded note payload on demand. `import_studio_clip` retains owner and saved-library authority across owner and editable contribution workspaces and updates manifest, normalized projection, snapshot, receipt, and lock in one transaction.
