...
- **Consequence:** MIDI-01 freezes the descriptor, adapter capability, identity, and clip contracts; MIDI-05 implements the composite runtime, normalized clip foundations, and atomic project-plus-empty-workspace command. The route/UI program follows MIDI-07 in four bounded slices before PR 18. Existing v1 revisions are never rewritten, only one source asset may back an initial audio track, and splitting is unavailable until normalized clip round trips are proven.
