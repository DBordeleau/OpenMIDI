...
OPT-04 adds optional source-bound `JSPK` v1 descriptors to that signed batch. Peak objects load concurrently, are digest/format/source/metadata validated inside this client boundary, and populate Waveform Playlist's supported `waveformData` before source decode. A missing or invalid hint is ignored; the placeholder remains until canonical audio decodes, and attaching the decoded buffer replaces persisted coarse peaks for normal zoom detail.
