...
PRs 01–17, OPT-01–OPT-05, MIDI-01–MIDI-07, and STUDIO-01–STUDIO-06 are implemented in the repository. Studio is the primary MIDI creation/recording/arrangement surface, with canonical routes, safe serial switching, a unified audio/MIDI arranger, deterministic mutation/history, integrated piano-roll recording, immutable collaboration flows, and tested compatibility for standalone MIDI and legacy audio. UX-01–UX-05 are the active bounded usability interruption; UX-01 is next, and its detailed local plan is `local/implementation-plans/023-studio-midi-usability-pass.md`. The hosted parity review and separately authorized source-admission lock remain later operational steps; source admission therefore remains enabled. PR 18 follows the usability pass and recorded capability state. User upload history intentionally excludes internal workspace snapshots. Moderation/retention operations and release hardening are not implemented. npm is the sole package manager and Node.js 24 LTS is required.
