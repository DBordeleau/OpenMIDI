...
PRs 01–14 and Phase C are implemented. The repository contains the responsive Next.js product shell with active-route navigation and an authenticated member-project index; progressively enhanced Auth-aware account links; invite-only Google OAuth and profile onboarding; private project metadata; immutable source assets uploaded directly and resumably to Supabase Storage and automatically trusted-verified through a durable, lease-bound Edge worker; explicit ordered source-credit confirmation; atomic publishing into immutable revisions with track-credit and publisher snapshots; authenticated, lazy-loaded Waveform Playlist playback; owner-only editable workspaces with optimistic, conflict-safe autosave and private recovery snapshots; canonical later-revision publishing from a saved workspace; stale-draft restart without automatic rebase; authorized direct-to-Storage source downloads; bounded browser-rendered 16-bit WAV mix export; private member contribution drafts with immutable submission versions; owner review with exact-version audition, request changes, rejection, and atomic acceptance; and durable accepted-contributor attribution with privacy-safe profile history. User upload history intentionally excludes internal workspace snapshots. Forks, discovery, moderation, and release hardening are not implemented. npm is the sole package manager and Node.js 24 LTS is required.
