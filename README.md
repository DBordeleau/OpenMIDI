...
OPT-01 adds deterministic large WAV generation and local browser benchmarks without committing media. OPT-02 extends the delivery harness with `--loader progressive` so shell timing and actor-scoped warm reuse are measured separately from full WAV playback readiness. Generated fixtures, disposable codec packages, and raw results remain under ignored `local/`. The checked-in method and bounded results are in the [OPT-01 baseline](docs/technical-design/evidence/opt-01-audio-delivery-baseline.md) and [OPT-02 progressive evidence](docs/technical-design/evidence/opt-02-progressive-studio.md). Start with `node scripts/generate-studio-audio-fixtures.mjs --profile controlled`; neither benchmark reads or mutates hosted Supabase.
