...
> **Current status:** PRs 01–17, the five-slice $0 audio-delivery optimization, and MIDI-01–MIDI-06 are complete. The repository includes frozen MIDI/manifest/session contracts, deterministic sample-free presets, accessible My stems composition/recording, immutable versions, exact project import/publication/preview/export, and complete MIDI contribution/acceptance/credit/fork paths while legacy audio remains supported. MIDI-07 transition readiness is next. Six studio-forward slices then establish the canonical Studio, unified audio/MIDI arranger, integrated piano-roll recording, and final parity/audio-lock gate before PR 18 resumes.
...
The accepted future route model is `/studio` for the authenticated start center and `/studio/{projectId}` for one selected authorized session. Those routes do not exist yet: MIDI-01 established the reusable contracts, MIDI-05 implemented the route-neutral resolver/composite runtime, and STUDIO-01 performs the route migration while retaining the current nested URL as a compatibility redirect. STUDIO-03–STUDIO-05 replace the current form-like MIDI surface with a shared timeline and integrate the existing piano roll/recorder into Studio; STUDIO-06 owns parity and final source-admission lock enablement. See the [roadmap](docs/ROADMAP.md) and [studio-forward plan](docs/studio-forward-refactor-plan.md).
