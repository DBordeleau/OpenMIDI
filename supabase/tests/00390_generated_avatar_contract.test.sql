...
select ok(exists(select 1 from information_schema.columns where table_schema='public' and table_name='profiles' and column_name='avatar_path'),'legacy avatar path remains during expand slice');
select ok(exists(select 1 from storage.buckets where id='profile-images') and exists(select 1 from storage.buckets where id='public-avatars'),'legacy avatar buckets remain during expand slice');
