          ...
          coalesce((
            select jsonb_agg(
              jsonb_build_object(
                ...
                -- Manifest tracks carry a name; clips do not. The dashboard
                -- miniature labels each lane with the track and each block with
                -- the pattern it places, so it reads as an arrangement instead
                -- of anonymous bars.
                ...
                'clips', coalesce((
                  select jsonb_agg(
                    jsonb_build_object(
                      ...
                      -- Already visible to this caller in the Studio: the clip
                      -- sits in a workspace they own.
                      'pattern_name', (
                        select mp.name
                        from public.midi_pattern_versions mpv
                        join public.midi_patterns mp
                          on mp.id = mpv.midi_pattern_id
                        where mpv.id = (clip.value ->> 'midiPatternVersionId')::uuid
                      )
                    )
                    order by
