...
function seedDiscoveryFixture() {
  const sql = `
    ...
    insert into public.arrangement_versions(id,project_id,created_by,create_request_id,manifest_version,engine,engine_version,manifest,manifest_sha256,tempo_bpm,time_signature_numerator,time_signature_denominator,musical_key,ppq,duration_ticks)
      values('${arrangementId}','${projectId}','${ownerId}','e5000000-0000-4000-8000-000000000011',3,'jam-session-midi','jam-session-midi-3_tone-15.1.22_presets-1',jsonb_build_object('manifestVersion',3,'engine','jam-session-midi','engineVersion','jam-session-midi-3_tone-15.1.22_presets-1','projectId','${projectId}','tempoBpm',120,'timeSignature',jsonb_build_object('numerator',4,'denominator',4),'musicalKey','c-minor','ppq',480,'durationTicks',960,'tracks',jsonb_build_array(jsonb_build_object('trackId','${trackId}','sortOrder',0,'name','Warm keys pulse','presetId','warm-keys','presetVersion',1,'gainDb',-6,'pan',0,'muted',false,'soloed',false,'clips',jsonb_build_array(jsonb_build_object('clipId','${clipId}','midiPatternVersionId','${patternVersionId}','startTick',0,'durationTicks',960,'sourceStartTick',0,'loop',false))))),repeat('a',64),120,4,4,'c-minor',480,960) on conflict (id) do nothing;
    ...
    insert into public.project_revisions(id,project_id,revision_number,created_by,publish_request_id,message,manifest,manifest_version,engine,engine_version,manifest_sha256,duration_ms,arrangement_version_id)
      select '${revisionId}','${projectId}',1,'${ownerId}','e2000000-0000-4000-8000-000000000011','First signal',manifest,3,'jam-session-midi','jam-session-midi-3_tone-15.1.22_presets-1',manifest_sha256,1000,'${arrangementId}' from public.arrangement_versions where id='${arrangementId}' on conflict (id) do nothing;
